...
For CLI and editor automation, use `tysel --error-format json dev`. Build and
runtime diagnostic updates are newline-delimited JSON on stderr; successful
reloads publish an empty diagnostic array to clear stale errors. Fatal setup
errors remain one `TYSEL_CLI_ERROR` object and exit `1`. Command reports go to
stdout, so parse the streams separately.
