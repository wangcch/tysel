...
| `permissions.redis` | Array, at most one item | `name`, `name:read-only`, or `name:read-write` | A named Redis grant whose URL is injected by the deployment. |
...
See [Permissions](permissions.md) for hostname semantics, Postgres and Redis environment
...
| Redis use | At most one named grant plus `TYSEL_REDIS_<NAME>` | Connection URLs and credentials stay outside the manifest. |
