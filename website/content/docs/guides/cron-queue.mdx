...
description: "This guide registers a UTC schedule and a named Queue consumer, invokes the Queue path from the CLI, and verifies the scheduler without introducing a second application protocol."
---
...
make an ordinary Queue or Cron handler durable. Use the [Durable API](/reference/runtime/durable)
...
See the [application task types](/reference/runtime/application),
[`tysel queue`](/reference/cli/tasks#tysel-queue), and
[task limits](/reference/limits-and-defaults#application-task-and-mcp-bounds).
