...
description: "This guide exposes a Tysel service deliberately: choose its listener and HTTP protocols, enable WebSocket upgrades, grant outbound destinations, and verify the failure paths before deployment."
---
...
Continue with [Concurrency and backpressure](/docs/guides/concurrency-backpressure), the
[server fields](/reference/manifest/app-server), and the
[network capability contract](/reference/manifest/permissions#outbound-network).
