...
description: "Choose the executable or OCI artifact boundary in Deployment selection before applying this runbook."
---

Choose the executable or OCI artifact boundary in
[Deployment selection](/docs/operations/deployment) before applying this runbook.
...
record. See [Reproducible release](/docs/guides/reproducible-release).
...
| Startup signal | Log contains <code>{"tysel listen <address>"}</code>. |
...
| PostgreSQL grant `NAME` | <code>{"TYSEL_POSTGRES_<NAME>"}</code> | Manifest or image |
...
See [Observability](/docs/guides/observability) for exact signals and redaction.
...
before closing the incident. See [Debugging](/docs/guides/debugging).
