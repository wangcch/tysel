...
description: "HTTP handlers receive host methods through their runtime argument. The same object remains available as globalThis.tysel for compatibility and for task or durable handlers."
---
...
For the profile-by-profile matrix, see [Capabilities](/docs/capabilities).
...
result bytes are bounded; see [Limits and defaults](/reference/limits-and-defaults).
Use the [SQLite](/docs/guides/sqlite) or
[PostgreSQL](/docs/guides/postgresql) guide for an end-to-end deployment path.
...
connections are bounded. See the [Redis guide](/docs/guides/redis).
...
Use the [filesystem guide](/docs/guides/filesystem) to prepare roots and
...
model aliases, and secret selection come from [environment variables](/reference/environment).
