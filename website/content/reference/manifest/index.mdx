...
See [Permissions](/reference/manifest/permissions) for hostname semantics, Postgres and Redis environment
...
| Redis use | At most one named grant plus <code>{"TYSEL_REDIS_<NAME>"}</code> | Connection URLs and credentials stay outside the manifest. |
