---
title: "Permissions"
description: "The optional [permissions] table grants host capabilities. Every list defaults to empty, so access is denied unless declared."
---
...
Follow the [PostgreSQL guide](/docs/guides/postgresql) for parameter types,
...
before connecting. See the [Redis guide](/docs/guides/redis).
...
Follow the [filesystem guide](/docs/guides/filesystem) for root preparation,
path confinement, profile differences, denial checks, and a runnable transform.

See [Host capabilities](/reference/runtime/capabilities) for method signatures,
[Environment variables](/reference/environment) for host configuration, and the
[Security model](/docs/security) for enforcement boundaries.
