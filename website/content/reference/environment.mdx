...
description: "Environment variables configure the host and installation, not application JavaScript."
---
...
| --- | --- | --- |
| <code>{"TYSEL_POSTGRES_<NAME>"}</code> | The manifest grants named Postgres access | Connection URL for the one configured grant. Uppercase the name and replace `-` with `_`. |
| <code>{"TYSEL_REDIS_<NAME>"}</code> | The manifest grants named Redis access | Connection URL for the one configured grant. Uppercase the name and replace `-` with `_`. |
...
[Component capabilities](/reference/component/capabilities) for the three-layer
intersection and local `tysel run` behavior.

See [Permissions](/reference/manifest/permissions), [Host capabilities](/reference/runtime/capabilities),
[Observability guide](/docs/guides/observability), and
