...
| <code>{"--entry <path>"}</code> | Project-relative path | Reuse an existing JavaScript or TypeScript entry. A newly generated entry must use `.ts`, `.tsx`, or `.mts`; `.wasm` is rejected because `init` does not yet generate Component projects. |
