...
- Production refresh: website CI / `pnpm sync:benchmarks` downloads a fresh artifact from the canonical successful **main-branch push** CI workflow. Full CI runs bind the evidence commit to the workflow head SHA and validate all seven gates. Docs-only runs reuse the latest fresh, validated main evidence because no runtime inputs changed.
